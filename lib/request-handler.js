...
class RequestHandler {
  ...
  async _switchToNextAuth() {
    ...
    if (this.startOfFailureCycleIndex !== null && nextAuthIndex === this.startOfFailureCycleIndex) {
        ...
        throw new Error('全账号循环失败，熔断机制已激活。');
    }
  ...
  async _handleRequestFailureAndSwitch(errorDetails, res) {
    if (this.config.debugMode) {
      ...
      await this.browserManager.takeScreenshot(`status-${errorDetails.status}`);
    }
  ...
  async processRequest(req, res) {
    if (this.fullCycleFailure) {
      this.logger.error('🔴 [熔断] 系统处于熔断状态，拒绝新的 API 请求。');
      return this._sendErrorResponse(res, 503, '服务暂时不可用，因为所有备用账号均已尝试失败 (熔断已触发)。');
    }
    if ((!this.config.apiKeys || this.config.apiKeys.length === 0) && req.query && Object.prototype.hasOwnProperty.call(req.query, 'key')) {
      if (this.config.debugMode) {
    ...
    try {
      ...
      this.logger.error(`[请求处理] processRequest 顶层捕获到未处理的异常: ${error.message}`);
      this._sendErrorResponse(res, 500, `代理服务器内部错误: ${error.message}`);
    } finally {
  ...
  async _handleRealStreamResponse(proxyRequest, messageQueue, req, res) {
    this.logger.info(`[请求] 真实流式处理流程启动，路径: "${req.path}"`);
    ...
    const keepAliveChunk = this._getKeepAliveChunk(req);
    const connectionMaintainer = setInterval(() => { if (!res.writableEnded) res.write(keepAliveChunk); }, 2000);

    try {
      // 在真实流模式下，我们假设一次尝试，失败则直接抛出错误
      this._forwardRequest(proxyRequest);
      ...
      while (!streamEnded) {
        ...
        if (message.event_type === 'error') {
          const correctedMessage = this._parseAndCorrectErrorDetails(message);
          await this._handleRequestFailureAndSwitch(correctedMessage, res);
          // 抛出错误以中断流程并由外层捕获
          throw new Error(`Stream error from browser: ${correctedMessage.message}`);
        }

        if (message.type === 'STREAM_CHUNK') {
          if (!res.writableEnded) res.write(`data: ${message.data}\n\n`);
        }
      ...
      // 如果循环正常结束，说明请求成功
      if (this.failureCount > 0) {
        this.logger.info(`✅ [认证] 真实流请求成功 - 失败计数已从 ${this.failureCount} 重置为 0，熔断状态已重置。`);
        this.failureCount = 0;
        this.fullCycleFailure = false;
        this.startOfFailureCycleIndex = null;
      } else {
        this.logger.info(`✅ [认证] 真实流请求成功。`);
      }
    } catch (error) {
        this.logger.error(`[请求] 真实流处理期间发生错误: ${error.message}`);
        if (!res.headersSent || !res.writableEnded) {
            this._sendErrorChunkToClient(res, `处理失败: ${error.message}`);
        }
        // 将错误重新抛出，以便 processRequest 的 catch 块可以处理它
        throw error;
    } finally {
      clearInterval(connectionMaintainer);
      if (!res.writableEnded) res.end();
      this.logger.info('[请求] 真实流响应处理结束。');
    }
  }
  ...
  _sendErrorResponse(res, statusCode, message) {
    if (res.headersSent) {
      this.logger.warn(`[错误处理] 响应头已发送，无法发送状态码 ${statusCode} 的错误。`);
      return;
    }
