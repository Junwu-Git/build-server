    ...
    try {
      ...
      for (let attempt = 1; attempt <= maxNavRetries; attempt++) {
        try {
          ...
          // 在关闭过程中，页面对象可能不存在，需要检查
          if (this.isClosing || !this.page) {
            this.logger.warn('[浏览器] 启动被中断，跳过失败截图。');
            throw new Error('启动被中断'); // 抛出新错误以停止重试循环
          }
          if (this.config.debugMode) {
            const errorScreenshotPath = path.join(debugFolder, `failed-nav-${authIndex}-${attempt}-${new Date().toISOString().replace(/:/g, '-')}.png`);
            await this.page.screenshot({ path: errorScreenshotPath, fullPage: true }).catch(err => {
              this.logger.warn(`[调试] 尝试保存失败截图时出错: ${err.message}`);
            });
      ...
      // 检查点：在执行任何后续页面操作前，检查是否正在关闭
      if (this.isClosing) { this.logger.info('[浏览器] 启动在页面加载后被中断，提前退出。'); return; }

      this.logger.info('[浏览器] 页面加载完成，无条件等待10秒，确保UI完全稳定...');
      await this.page.waitForTimeout(10000);

      if (this.isClosing) { this.logger.info('[浏览器] 启动在初次等待后被中断，提前退出。'); return; }
      ...
      if (combinedSelector) {
        while (Date.now() < cleanupTimeout && !this.isClosing) { // 在循环中也加入检查
            const buttons = await this.page.locator(combinedSelector).all();
            for (const button of buttons) {
                if (this.isClosing) break; // 在内层循环也检查
      ...
      if (this.isClosing) { this.logger.info('[浏览器] 启动在弹窗清理后被中断，提前退出。'); return; }
      ...
      try {
        ...
        if (this.isClosing) { this.logger.info('[浏览器] 启动在点击按钮前被中断，提前退出。'); return; }
        await codeButton.click({ force: true });
        this.logger.info(`[浏览器] 已成功强制点击 "${name}" 按钮。`);
      } catch (err) {
        if (this.isClosing) { this.logger.info('[浏览器] 启动在点击按钮期间被中断，属正常行为。'); return; }
      ...
      // 如果错误是由于中断引起的，则静默处理，否则记录为真实错误
      if (error.message.includes('启动被中断')) {
        this.logger.info(`[浏览器] 账号 ${authIndex} 的启动流程被优雅中断。`);
      } else {
