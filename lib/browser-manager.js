...
class BrowserManager {
    ...
    try {
      ...
      for (let attempt = 1; attempt <= maxNavRetries; attempt++) {
        try {
          ...
          if (this.config.debugMode) {
            this.logger.info(`[诊断] 在 page.goto catch 块中。isClosing: ${this.isClosing}, page 存在: ${!!this.page}`);
            ...
            if (this.page) {
              ...
              this.logger.warn('[诊断] 页面对象不存在，跳过截图。');
            }
          }
      ...
      this.logger.info(`[诊断] 在 launchBrowser 的最外层 catch 块中。isClosing: ${this.isClosing}`);
      if (this.browser) {
        await this.closeBrowser(); // 使用封装好的关闭方法
      }
      throw error;
    }
  }
  ...
  async closeBrowser() {
    this.logger.info(`[诊断] closeBrowser 被调用。当前浏览器实例是否存在: ${!!this.browser}`);
    this.isClosing = true; // 立即设置标志
    if (this.browser) {
