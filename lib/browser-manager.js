...
class BrowserManager {
  constructor(logger, config, authSource) {
    ...
    // BrowserManager 现在使用 Playwright 的内置浏览器管理, 无需指定路径。
    // 用户需要在首次运行时执行 `npx playwright install firefox`。
    this.logger.info('[系统] BrowserManager 将使用 Playwright 管理的浏览器版本。');
  }
  ...
  async launchBrowser(authIndex) {
    ...
    this.logger.info(`   • 浏览器: Playwright 内置 Firefox`);
    this.logger.info('==================================================');
    ...
    try {
      this.browser = await firefox.launch({
        headless: true,
        // executablePath 已移除, Playwright 将自动使用其管理的浏览器
